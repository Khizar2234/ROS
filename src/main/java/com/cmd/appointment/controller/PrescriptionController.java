...
public class PrescriptionController {
	...
	@GetMapping("/ViewPrescriptions/{aid}")
	@Operation(summary = "To view all prescription")
	public ResponseEntity<?> getPrescriptions(@RequestParam (value = "appointment_id") long appointment_id) {
		ResponseEntity<?> response = null;
		try {
			response =  new ResponseEntity<>(presRepo.viewPrescriptionsByAppId(appointment_id), HttpStatus.OK);
